...
def getpgd(addr):
    if (addr&0xf000000000000000) == 0xc000000000000000:
        ...
        curpgd=read_addr8(sock, sym['pgd_current'])
    return curpgd

pgdorder = 0

def accesshelper(head,addr,tail,pgd):
	pgdmask = (((1+pgdorder)<<pageshift)-8);
        mask = ((1<<pageshift)-8);
...
# 11 = (14-3)
# 12-2
        pud=read_addr8(sock,((addr>>((pageshift-3)*3))&pgdmask) + pgd);
        pmd=read_addr8(sock,((addr>>((pageshift-3)*2))&mask) + pud);
        pte=read_addr8(sock,((addr>>(pageshift-3)) & mask1) + pmd);
        pte1=read_addr8(sock,((addr>>(pageshift-3)) & mask1) + 8 + pmd);
        #printf("pmd=%x pte=%x pte1=%x\n", $pmd, $pte, $pte1);


        if addr&(1<<pageshift):
            if pte1&(2<<pteshift):
                addr1 = (0x9800000000000000,0x9000000000000000)[((pte1>>(pteshift+3))&7)!=3] + ((pte1>>(pteshift+6))<<12) + (addr&((1<<pageshift)-1));
            else :
            	return None;
        else :
            if pte&(2<<pteshift) :
                addr1 = (0x9800000000000000,0x9000000000000000)[((pte>>(pteshift+3))&7)!=3]  + ((pte>>(pteshift+6))<<12)  + (addr&((1<<pageshift)-1));
            else :
             	return None;
...
def probepte():
    ...
    if debuginfo: print( 'g: pteshift:'+str(pteshift)+'\n')
                      ...
                      try:
                        if testre(re.search(r'\$(m|M)([^,]+)(,[^#]+)#',buf),m):
                            ...
                            if  ((addr>>32)&0xf0000000)==0xc0000000:
                                ...
                                buf = accesshelper(cmd,addr - 0xc000000000000000, tail,pgd);
                            elif addr<0x8000000000000000:
                                buf = accesshelper(cmd, addr, tail, getpgd(addr));
                            elif ((addr>>32)==0xffffffff and (addr&0xc0000000)==0xc0000000):
                                pgd=sym['swapper_pg_dir'];
